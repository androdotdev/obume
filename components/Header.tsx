...
export function Header({ title, backHref, subtitle, rightElement }: HeaderProps) {
  return (
    <div className="flex items-center justify-between mb-8 md:mb-9">
      <Link
        href={backHref}
      ...
      <div className="text-right">
        <h1 className="m-0 text-2xl md:text-[28px] font-bold text-slate-100 tracking-tight font-serif">
          {title}
        </h1>
        {subtitle && (
          <span className="text-[13px] md:text-[12px] text-slate-600">{subtitle}</span>
        )}
      </div>
